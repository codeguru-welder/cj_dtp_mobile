(function (factory) {
	...
	var _mp = window.miapsPage = {
		defaultUrl : location.origin + "/miapsdemo/",
				...
				if (typeof idx === 'number') {
					if (idx == -1) {
						...
						if (idx > _mp.hArray.length - 1) {
							if (_tmdebug) console.log('index (' + idx + ') is greater than "hArray.length"');
							return null;
						}					
						for (var i=0; i<idx; i++) {
							_mp.hArray.pop();						
					...
					if (i == -1) { // 찾는 페이지ID가 없을 경우
						if (_tmdebug) console.log('ID (' + idx + ') is not exist in "hArray"');
						return null;
					}
		...
		go : function() { // id:string, *url:string, data:object, type:string('Push','Replace'), // object
			...
			if (typeof arguments[0] === "object") {
				...
				if (!_obj.hasOwnProperty('url')) {
					if(_tmdebug) { console.log ('url은 필수 입력값 입니다.'); }
					return;
				}
				...
				if (_obj.hasOwnProperty('data')) {
					if (typeof _obj.data === 'object') {
						...
						if (_tmdebug) console.log('전달 데이터는 Object Type입니다.');
						return;
					}
				}
				if (_obj.hasOwnProperty('type')) {
					...
					if (type != "push" && type != 'replace') {
						if (_tmdebug) console.log('Type은 "Push" 또는 "Replace" 입니다.');
						return;
					}
		...
		back : function() { // number: -int or id:string, data:object
			...
			if (arguments.length == 2) {		// number or id, data
				...
				if (typeof arguments[0] === 'string') {	// id
					...
					if (_tmdebug) console.log('전달 데이터는 Number 또는 String(id) 또는 Object(data) Type입니다.');
					return;					
				}
			...
			if (miaps.isPC()) {
				...
				if (cfg.param.hasOwnProperty('id')) {
					...
					if (cfg.param.size > _mp.hist.getSize()) {
						if (_tmdebug) console.log("전체 히스토리보다 큰 수를 입력 했습니다.");
						return;
					}
				...
				miaps.mobile(cfg, function(res) {
					if (_tmdebug) console.log(res);
					if (res.code != 200) {
						// 뒤로 가기 실패
						if (_tmdebug) console.log("뒤로가기에 실패 했습니다.");
						return;						
					}
				});
			}
		...
		goTopPage : function() { // nothing or data:object
			...
			if (miaps.isPC()) {
				...
				miaps.mobile(cfg, function(res) {
					...
					if (res.code != 200) {
						// 뒤로 가기 실패
						if (_tmdebug) console.log("goTopPage() 실패 했습니다.");
						return;						
					}
				});
			}
		...
		getPopupData : function(cbfunc) { // cbfunc:function, data:object(optional)
	        var cfg = {
		        type: "popup",
			...
			// 2021.04.27 chlee, closePopup 파라메터를  getPopupData에 전달 후 다시 받기 위해 추가. 
			if (arguments.length > 1 && typeof arguments[1] === "object") {
				cfg.param.closedata = arguments[1];
			}
		...
		openPopup : function() { // *url:string, data:object
			...
			if (miaps.isPC()) {				
				...
				//var popobj = window.open(cfg.param.url, cfg.param.id, '', true); // A boolean is being passed as a fourth parameter to window.open. This is not used and may cause an exception in a future release.
				var popobj = window.open(cfg.param.url, cfg.param.id, ''); // popup id를 이름으로 창을 열고 추후 이름으로 창에 접근할 수 있다.
		...
		closePopup : function() { // data:object
			...
			// 2021.04.27 chlee, 닫을 팝업의 ID를 얻어서 부모에게 보낸다.
			miapsPage.getPopupData(function(data) {
				var popInfo = miaps.parse(data);
				var popId = popInfo.res.id;
				...
				if (popInfo.param.hasOwnProperty('closedata')) {
					cfg.param.data = popInfo.param.closedata;					
				}
					...
					if (obj.res.length == 0) {
						miapsPage.getHistoryCount(function(data) {
							...
							if (obj2.res.cnt == 1) { // 히스토리 수가 1일 경우
								showModal('종료 하시겠습니까?', 'confirm', function() {
									miaps.mobile({									
										type: 'exit',
										param: ''
									}, null);
