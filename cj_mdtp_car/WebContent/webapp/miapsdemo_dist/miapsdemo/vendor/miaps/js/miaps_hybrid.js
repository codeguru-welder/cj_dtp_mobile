...
(function (window, factory) {
	if (window._debug == undefined) { window._debug = true;}
	...
	var MiapsHybrid = {
		 ...
		 */
		miapsSvc : function() {
			var classname = '', methodname = '', target = '', params = '', cb_func_nm = '', cb_func = '', pathname = '', headerObj = '';
				...
				if(window._svc == 'MiAPS') {
					...
					if (MiapsHybrid.MobilePlatform.Android()) {
						...
						if (pathname != null && pathname != '') {
							...
							window.MiAPS.runMiaps(MiapsHybrid.defServlet, classname, methodname, target, encodeURIComponent(sendParam), encodeURIComponent(header), encodeURIComponent(cb_func_nm));
						}
					/* iPhone/iPad */
					} else if (MiapsHybrid.MobilePlatform.iPhone() || MiapsHybrid.MobilePlatform.iPad()) {
						...
						if (pathname != null && pathname != '') {
							scheme += "&pathname=" + pathname; // application.json내 serverurl에 url만 있어야합니다.
						} else {
							scheme += "&pathname=" + MiapsHybrid.defServlet;
						}
						scheme += "&params=" + encodeURIComponent(sendParam)
		 ...
		 * @param cb_func 웹에서 호출 할 콜백함수, Object
		 */
		querySvc : function(mode, target, query, cb_func_nm, cb_func) {
			MiapsHybrid.resultData = null;
			if (MiapsHybrid.NaviPlatform.Windows() || MiapsHybrid.NaviPlatform.Mac()) {
				if (window._debug) {
					console.log("mode:" + mode + ", target:" + target + ", query:" + query + ", cbfunc:" + cb_func);
				}
	...
	MiapsHybrid.mobile({type:'devicemodel', param:'PC'}, function(data) {
		...
		MiapsHybrid.mh_model = obj.res;		
	});

	MiapsHybrid.mobile({type:'platformversion', param:'Windows10'}, function(data) {
		var obj2 = MiapsHybrid.parse(data);
		MiapsHybrid.mh_os = obj2.res;
	});
