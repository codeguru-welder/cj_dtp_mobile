...
$(document).ready(function(){
	...
	var sv = "<c:out value='${search.searchValue}'/>"; 
	if ("_^&%$_" == sv || "_^&amp;%$_" == sv || "_^&amp;amp;%$_" == sv) {
		$("#searchValue").val("");
	} else {
		$("#searchValue").val(sv);
	}
...
function enterSearch(obj) {
	...
	if(document.searchFrm.searchValue.value == '') {
		return;
	}
	
	$("#searchFrm").find("input[name='startRow']").val("0");
	$('#listUl').empty();
	goList();
}
...
function goList() {
	...
	$.post('appClientListSearchView.miaps?', $(searchFrm).serialize(), function(data){
...
<body>
<div class="appcenter-header"><mink:message code='mink.label.search_app'/></div>
<div>
	<form id="searchFrm" name="searchFrm" method="get" onSubmit="return false;">
		...
		<input type="hidden" name="searchKey" value="appNm" />
		
		<div class="searchArea">
			<input name="searchValue" type="text" id="searchValue" onkeypress="if(event.keyCode == 13) enterSearch(this);"class="searchInput" style="border: solid 1px #dedede;">
 		</div>
