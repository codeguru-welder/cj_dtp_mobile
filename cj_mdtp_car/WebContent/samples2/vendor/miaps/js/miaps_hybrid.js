...
(function (window, factory) {
	if (window._tmdebug == undefined) { window._tmdebug = false;} // true, false
	...
	var pathConfig = {
		...
		myFileRoot: "/samples2/"					// 개인 프로젝트별로 재정의 (hybrid local path, Ex: /hybrid/)
	}
	...
	//if (window._tmdebug) { console.log(tmpStr); }
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        // like Node.
		if(navigator.platform.match(/Win32/i) || navigator.platform.match(/Mac/i)) {
			module.exports = factory(resPath, extlibPath);
    	} else {
    		module.exports = factory(resPath, null);
    	}
    } else {
	...
	var MiapsHybrid = {
		...
		mh_os: '',
		maxLogLength: 2000,	
		serverInfo : [], //	ex) {"url":"http://miaps.thinkm.co.kr/miaps6beta","chkPath": "/miapsdemo2/","runmode": "real"},{},..
		sendUrl : '',
	  	MobilePlatform : {
		    Android: function () {
		...
		NaviPlatform : {
			...
			Mac: function() { // MacIntel
				...
				//return navigator.platform.match(/MacIntel|MacPPC|Mac68K/i);
		...
		updateApp : function(params, cb_func) {
			if (MiapsHybrid.NaviPlatform.Windows() || MiapsHybrid.NaviPlatform.Mac()) {
				...
				if (window._pccmmtype == 'DWR') {
					...
					if (window._tmdebug) { console.log('http://'+ location.hostname + ':' + location.port + _gContextNm + '/hybrid/updateApp.miaps'); }
		...
		miapsSvcSp: function () {
			...
			if  (callback == null || callback == '') {
				...
				if (callback.name == null || callback.name == '') { // 콜백명이 없으면 임시로 만든다.					
					...
					if (window._tmdebug) { console.log(tmpCallbackNm); }
					
					window[tmpCallbackNm] = callback;
					if (window._tmdebug) { console.log(window[tmpCallbackNm]); }
		...
		miapsSvcEx : function(obj, cb_func) { // param:object, callback:function
			if (typeof obj != 'object') {
				if (_tmdebug) { console.log('전달 파라메터는 object type이여야 합니다.'); }				
				return;
			}
			/* {
				class: '',			// fixed
		...
		httpSvc : function(config, cb_func) {
			...
			if (cb_func) { // object callback 함수는 있고 config.callback이 없으면 임시로 만든다.
				if (!config.hasOwnProperty('callback')) {
					if (window._tmdebug) { console.log(cb_func.name); }
					...
					if (cb_func.name == null || cb_func.name == '') { // 콜백명이 없으면 임시로 만든다.
						...
						if (window._tmdebug) { console.log(window[cb_func.name]); }
			...
			if (cb_func) { // object callback 함수는 있고 config.callback이 없으면 임시로 만든다.
				if (!config.hasOwnProperty('callback')) {
					...
					if (cb_func.name == null || cb_func.name == '') { // 콜백명이 없으면 임시로 만든다.
						...
						if (window._tmdebug) { console.log(JSON.stringify(config) + ', callback : ' + tmpCallbackNm); }
						...
						window[cb_func.name] = cb_func;
						if (window._tmdebug) { console.log(JSON.stringify(config) + ', callback : ' + cb_func.name); }
						config.callback = cb_func.name;
					}
				}
			} else {
				if (window._tmdebug) { console.log(JSON.stringify(config) + ', callback : none'); }
				config.callback = '';
			}
	...
	if (_tmdebug == true && (MiapsHybrid.isAndroid() || MiapsHybrid.isIOS())) {	
	...
	if (_tmdebug == true && MiapsHybrid.isWebView()) {
