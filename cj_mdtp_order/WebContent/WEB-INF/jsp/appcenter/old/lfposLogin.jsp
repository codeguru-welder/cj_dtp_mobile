...
//			document.getElementById("enpw").focus();
		//} else {
			document.getElementById("posShopCode").focus();
		//}
	}
...
//			f.platformName.value = "2000013";
	...
	function login() {
		...
		if(form.posShopCode.value == "") {
			alert("매장코드를 입력해주세요.");
			return;
		}
		
		if(form.posMobileNum.value == "" || isNaN(form.posMobileNum.value) == true) {
			alert("전화번호를 정확히 입력해주세요.");
			return;
		}
		
		if(form.posPassword.value == "") {
			alert("패스워드를 입력해주세요.");
			return;
		}
		
		form.action = "lfposLoginCheckM.miaps";
		form.submit();
	}
...
<div id="mobileWrap">
	...
	<div id="contentsWrap">	
		...
		<!-- <input type="hidden" name="packageNm" id="packageNm" value="com.lfcorp.miaps.posqa"/> -->
			<fieldset>
			<legend>Login</legend>
				...
				<div class="login_wrap">
					<dl>
			       		<c:if test="${validUser == 'N'}">
							<p class="notice_save2" ><mink:message code='mink.message.not_match_id_pwd' text='<font color=red>입력하신 매장코드 혹은 비밀번호가  
								<br />일치하지 않습니다. </font>'/>
							</p>
			       		</c:if>
					</dl>
					<dl>
						...
						<dd><input type="text" name=posShopCode id="posShopCode" maxlength="30" value="" tabindex="1"  title="kunnr" /></dd>
						<dt><label for="id">전화번호</label></dt>
						<dd><input type="text" name="posMobileNum" id="posMobileNum" maxlength="30" value="" tabindex="1"  title="tel" /></dd>
						...
						<dd>
							...
							<input type="password" name="posPassword" id="posPassword" autocomplete="off" maxlength="32" value="" tabindex="2" title="pwd" />
						</dd>
					</dl>
					<p class="id_save">
