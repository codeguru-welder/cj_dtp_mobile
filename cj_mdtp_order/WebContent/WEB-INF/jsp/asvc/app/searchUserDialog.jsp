...
<div>
	<div id="searchUserDialog" title="<mink:message code='mink.web.text.info.user'/>" class="dlgStyle">
		...
		<div id="searchUserDialogTitle" class="dialogTitle">
			<mink:message code="mink.web.text.regist.menumember.user"/>
		</div>
		...
		<div class="searchUserListDialogScroll">
			<table cellspacing="0" cellpadding="0" width="100%" class="listTb">
...
$(function(){
	...
	var insertMenuRoleUrl = "appRoleInsert.miaps?";
	/***/
	...
	var insert = function() {
		...
		$.post(insertMenuRoleUrl, $frm.serialize(), function(data){
			...
			//if($(":input[name='pageNum']").length < 1) goList(); // 바로 검색
			//else goDetail(appId); // 바로 검색
			...
			if(data.userList.length < 1) {
				...
				for(var i = 0; i < data.userList.length; i++) {
					...
					*/
					...
					$tr.find("td:eq(2)").attr("align", "left").html("<a href=\"javascript: void(0);\" onclick=\"javascript:showSearchUserDetailDialog('"+userNo+"')\">"+userId+"</a>");
	...
	var showSearchUserDialog = function(event) {
		...
		$(titleId).html("<mink:message code='mink.web.text.regist.menumember.user'/>");
		$(tbodyId).html("");
		$(tfootId).show();
