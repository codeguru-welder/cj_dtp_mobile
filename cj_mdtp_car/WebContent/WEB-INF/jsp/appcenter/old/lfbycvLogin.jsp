...
//			document.getElementById("enpw").focus();
		//} else {
			document.getElementById("kunnr").focus();
		//}
	}
...
//			f.platformName.value = "2000013";
	...
	function login() {
		...
		form.ZMGRAS.value = form.kunnr.value; // 매장코드
		
		// 파라메타 유실방지용
		var projectNm = form.projectNm.value;
		var exLogin = form.exLogin.value;
		var platformName = form.platformName.value;
		var params = "?projectNm=" + projectNm + "&exLogin=" + exLogin + "&platformName=" + platformName;
		
		form.action = "oldMinkLogin_checkM.miaps" + params;
		form.submit();
	}
...
<div id="mobileWrap">
	...
	<div id="contentsWrap">	
		<form name="mink_login_form" id="loginform" method="post" action="minkLogin_checkM.do" >
		<input type="hidden" name="platformName" id="platformName"/>
		<input type="hidden" name="packageNm" id="packageNm" value="com.lfcorp.miaps.bycv"/>
		<input type="hidden" name="ZMGRAS" id="ZMGRAS" />
		<input type="hidden" name="exLogin" id="exLogin" value="lfbycvLoginProcessor"/>
		<input type="hidden" name="projectNm" id="projectNm" value="lfbycv"/>
			<fieldset>
			<legend>Login</legend>
				...
				<div class="login_wrap">
					<dl>
			       		<c:if test="${validUser == 'N'}">
							<p class="notice_save2" ><font color=red>입력하신 매장코드가 일치하지 않거나<br />바잉컨벤션 대상 매장이 아닙니다.</font>
							</p>
			       		</c:if>
					</dl>
					<dl>
						...
						<dd>
							<input type="text" name="kunnr" id="kunnr" maxlength="30" value="" tabindex="1"  title="kunnr" onkeypress="if(event.keyCode == 13) login();" />
							<input type="hidden" name="url" id="url" value="" />
							<input type="hidden" name="relative" id="relative" value="" />
							<input type="hidden" name="brand" id="brand" value="" />
							<input type="hidden" name="platform" id="platform" value="" />
							<input type="hidden" name="mobilefull" id="mobilefull" value="1" />
						</dd>
					</dl>
					<p class="id_save">
						<%-- <input type="checkbox"  name="saved_id" id="sid" tabindex="3" onclick="return saveId();" /><label id="idsave" for="sid" onclick="return saveId();"><mink:message code='mink.label.id_save' text='아이디 저장'/></label> --%>
					</p>
				</div>
				<p class="notice_save3" >매장코드를 입력하세요.</p>
	...
	<div id="footerWrap" ><p class="copy">Copyright ⓒ 2014 ThinkM All rights reserved.
