	 ...
	 */
     function goStartMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2_enUS.png") < 0 ){
    		 welBtnConfirm.attr("src","./img/btn_confirm2_enUS.png");
     ...
     function goReadyMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2_enUS.png") >= 0 ){
	    	 ...
	    	 welBtnConfirm.attr("src","./img/btn_confirm_enUS.png");
    	 }
     }   
    ...
    function drop(ev) {
		...
		if (nImageFileCount >= 10){
			alert("Up to 10 images can be registered.");
			return;
		}
		...
		if(typeof ev.dataTransfer.files == 'undefined'){
			...
			if (!!files && nCount === 0){
				...
				alert("Not a normal attachment.");
				return ;
			}
			
			for (var i = 0, j = nImageFileCount ; i < nCount ; i++){
				if (!rFilter.test(files[i].type)) {
					alert("Only image files (jpg, gif, png, bmp) can be uploaded.");
				} else if(files[i].size > nMaxImageSize){
					alert("The image size can not exceed 10 MB.");
				} else {
					//제한된 수만 업로드 가능.
					if ( j < nMaxImageCount ){
    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
			alert("Can not register because the total image size exceeds 50 MB. \n\n (file name : "+sFileName+", size : "+sFileSize+")");
		} else {
			//이미지 정보 저장							
	        ...
	        aFileList.push('	<a onclick="delImage(\'img'+nImageInfoCnt+'\')"><img class="del_button" src="./img/btn_del.png"  width="14" height="13" alt="delete attached photo"></a>');
			aFileList.push('	</li> ');   
    ...
    function html5Upload() {	
    	...
    	sUploadURL = '../../../asvc/board/boardImageInsertHtml5.miaps';
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				if (res.readyState() == 4) {
					if(sResString.indexOf("NOTALLOW_") > -1){
						...
						alert("Only image files (jpg, gif, png, bmp) can be uploaded. ("+sFileName+")");
					}else{
	...
	function onAjaxError (){
		alert("[Guide] You need the URL of the server where you want to upload photos.-onAjaxError");
	}
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			...
 			sUrl  : '../../../asvc/board/boardImageInsert.miaps',
 			sCallback : '/se2/sample/photo_uploader/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 	    	sFiletype : "*.jpg;*.png;*.bmp;*.gif",						//허용할 파일의 형식. ex) "*", "*.*", "*.jpg", 구분자(;)	
 	    	sMsgNotAllowedExt : 'Only JPG, GIF, PNG, BMP extensions are allowed',	//허용할 파일의 형식이 아닌경우에 띄워주는 경고창의 문구
